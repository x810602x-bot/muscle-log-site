...
import { $, esc, fmt, localDate, addDays, dailyTargets } from './util.js?v=087c7e0';
import { openDate } from './today.js?v=087c7e0';

// 每條線數字大小差很多，所以各自一格、上下排，共用同一條時間軸
// daily = 從每天吃的東西算出來的（有每日目標虛線）；其餘都是歐姆龍的欄位。off = 圖上預設先不勾（全勾圖會太長），表格一律都列
const SERIES = [
  { key: 'protein', label: '蛋白質', unit: 'g', color: '--series-1', digits: 1, labelDigits: 0, daily: 'protein_g' }, // 每天一點很密，點上的數字取整數才塞得下
  { key: 'kcal', label: '熱量', unit: 'kcal', color: '--series-9', digits: 0, daily: 'kcal' },
...
const BODY_SERIES = SERIES.filter((s) => !s.daily);
...
export async function show() {
  ...
  const points = {};
  for (const { key, daily } of SERIES.filter((s) => s.daily)) {
    points[key] = totals.map((t) => ({ x: new Date(`${t.log_date}T00:00`).getTime(), y: Number(t[daily]) }));
  }
  ...
  renderToggles();
  renderChart(points, dailyTargets(settings, latest?.weight), since);
...
function renderChart(points, targets, since) {
  ...
  for (const s of visible.filter((v) => v.daily && targets[v.daily] != null)) {
    const target = targets[s.daily];
    datasets.push({
